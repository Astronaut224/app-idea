<template>
  <div :style="{ display: 'flex', 'justify-content': 'center' }" class="bar">
    <div
      class="slider-box"
      :style="{ width: outerWidth + 40 + 'px', background: outBg }"
      @mousedown="handleClickZong"
    >
      <div
        :class="['slider-nei', isClick ? 'transtion' : '']"
        :style="{
          width: moveDistanceTwo - moveDistanceOne + 'px',
          left: `${moveDistanceOne + 20}px`,
          background: neiBg,
        }"
      ></div>
      <!-- <div
    ...
    handleMousedownOne(e) {
      ...
      document.onmousemove = (e) => {
        if (this.isMove) {
          ...
          this.$emit('move1', this.radiusArr)
